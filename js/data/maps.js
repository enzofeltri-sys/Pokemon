// Cartes en tuiles (grille de caractères). Chaque caractère = un type de tuile.
// Légende: '#' mur/arbre (bloqué) | '.' chemin | '"' herbe haute (rencontre) |
// 'C'/'D' porte (téléportation) | 'H' tuile de soin | 'M' Poké Mart | 'P' Boîte PC |
// '<'/'>' sortie de route | '~' eau (bloquée, décor de bord de mer) |
// 'W' mur décoré (tableau, bloqué comme un mur) | 'F' sol décoré (plante) |
// 'R' sol décoré (tapis)
window.PKMN = window.PKMN || {};

PKMN.TILE_INFO = {
  "#": { blocked: true },
  ".": { blocked: false },
  '"': { blocked: false, grass: true },
  "C": { blocked: false, warp: true },
  "D": { blocked: false, warp: true },
  "H": { blocked: false, heal: true },
  "M": { blocked: false, mart: true },
  "P": { blocked: false, pc: true },
  "<": { blocked: false, warp: true },
  ">": { blocked: false, warp: true },
  "~": { blocked: true, water: true },
  "W": { blocked: true },
  "F": { blocked: false },
  "R": { blocked: false }
};

PKMN.MAPS = {
  town: {
    name: "Val Brumeux",
    tiles: [
      "##############",
      "#............#",
      "#.D..C..M.D..#",
      "#............#",
      "#............>",
      "#............#",
      "#............#",
      "#............#",
      "#............#",
      "##############"
    ],
    warps: {
      "5,2": { toMap: "center", x: 4, y: 4 },
      "2,2": { toMap: "lab", x: 3, y: 4 },
      "10,2": { toMap: "rivalhouse", x: 3, y: 4 },
      "13,4": { toMap: "route1", x: 1, y: 4 }
    },
    playerStart: { x: 6, y: 6 }
  },

  center: {
    name: "Centre Pokémon",
    indoor: true,
    tiles: [
      "########",
      "#F..R..#",
      "#..H.P.W",
      "#......#",
      "#......#",
      "####D###"
    ],
    warps: {
      "4,5": { toMap: "town", x: 5, y: 3 }
    }
  },

  lab: {
    name: "Laboratoire",
    indoor: true,
    tiles: [
      "########",
      "#F..R..#",
      "#......W",
      "#......#",
      "#......#",
      "####D###"
    ],
    warps: {
      "4,5": { toMap: "town", x: 2, y: 3 }
    }
  },

  rivalhouse: {
    name: "Maison",
    indoor: true,
    tiles: [
      "########",
      "#F..R..#",
      "#......W",
      "#......#",
      "#......#",
      "####D###"
    ],
    warps: {
      "4,5": { toMap: "town", x: 10, y: 3 }
    }
  },

  route1: {
    name: "Route 1",
    tiles: [
      "####################",
      "#..................#",
      '#..""""......""""..#',
      '#..""""......""""..#',
      "<..................>",
      '#..""""......""""..#',
      '#..""""......""""..#',
      "#..................#",
      "#..................#",
      "####################"
    ],
    warps: {
      "0,4": { toMap: "town", x: 12, y: 4 },
      "19,4": { toMap: "route2", x: 1, y: 4 }
    },
    encounterRate: 0.12,
    encounterTable: [
      { id: 16, min: 2, max: 4 },  // Roucool
      { id: 19, min: 2, max: 4 },  // Rattata
      { id: 13, min: 2, max: 3 },  // Aspicot
      { id: 10, min: 2, max: 3 },  // Chenipan
      { id: 29, min: 3, max: 5 },  // Nidoran F
      { id: 32, min: 3, max: 5 }   // Nidoran M
    ]
  },

  route2: {
    name: "Route 2",
    tiles: [
      "####################",
      "#..................#",
      '#.""""......""""...#',
      '#.""""......""""...#',
      "<..................>",
      '#.""""......""""...#',
      '#.""""......""""...#',
      "#..................#",
      "#..................#",
      "####################"
    ],
    warps: {
      "0,4": { toMap: "route1", x: 18, y: 4 },
      "19,4": { toMap: "town2", x: 1, y: 4 }
    },
    encounterRate: 0.14,
    encounterTable: [
      { id: 25, min: 5, max: 8 },  // Pikachu
      { id: 27, min: 5, max: 8 },  // Sabelette
      { id: 52, min: 5, max: 8 },  // Miaouss
      { id: 23, min: 5, max: 8 },  // Abo
      { id: 43, min: 5, max: 9 },  // Mystherbe
      { id: 77, min: 6, max: 9 }   // Ponyta
    ]
  },

  town2: {
    name: "Sylverive",
    tiles: [
      "##############",
      "#............#",
      "#..D....C....#",
      "#............#",
      "<............>",
      "#............#",
      "#............#",
      "#............#",
      "#............#",
      "##############"
    ],
    warps: {
      "0,4": { toMap: "route2", x: 18, y: 4 },
      "3,2": { toMap: "gym1", x: 4, y: 6 },
      "8,2": { toMap: "center2", x: 4, y: 4 },
      "13,4": { toMap: "route3", x: 1, y: 4 }
    }
  },

  center2: {
    name: "Centre Pokémon",
    indoor: true,
    tiles: [
      "########",
      "#F..R..#",
      "#..H.P.W",
      "#......#",
      "#......#",
      "####D###"
    ],
    warps: {
      "4,5": { toMap: "town2", x: 8, y: 3 }
    }
  },

  gym1: {
    name: "Arène du Vent",
    indoor: true,
    tiles: [
      "########",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "####D###"
    ],
    warps: {
      "4,7": { toMap: "town2", x: 3, y: 3 }
    }
  },

  route3: {
    name: "Route 3",
    tiles: [
      "####################",
      "#..................#",
      '#..""""......""""..#',
      '#..""""......""""..#',
      "<..................>",
      '#..""""......""""..#',
      '#..""""......""""..#',
      "#..................#",
      "#..................#",
      "####################"
    ],
    warps: {
      "0,4": { toMap: "town2", x: 13, y: 4 },
      "19,4": { toMap: "town3", x: 1, y: 4 }
    },
    encounterRate: 0.15,
    encounterTable: [
      { id: 77, min: 8,  max: 11 },  // Ponyta
      { id: 58, min: 9,  max: 12 },  // Caninos
      { id: 27, min: 7,  max: 10 },  // Sabelette
      { id: 74, min: 7,  max: 10 },  // Racaillou
      { id: 19, min: 6,  max: 9  }   // Rattata
    ]
  },

  town3: {
    name: "Braseforge",
    tiles: [
      "##############",
      "#............#",
      "#..D..C..M...#",
      "#............#",
      "<............>",
      "#............#",
      "#............#",
      "#............#",
      "#............#",
      "##############"
    ],
    warps: {
      "0,4": { toMap: "route3", x: 18, y: 4 },
      "3,2": { toMap: "gym2", x: 4, y: 6 },
      "6,2": { toMap: "center3", x: 4, y: 4 },
      "13,4": { toMap: "route4", x: 1, y: 4 }
    }
  },

  center3: {
    name: "Centre Pokémon",
    indoor: true,
    tiles: [
      "########",
      "#F..R..#",
      "#..H.P.W",
      "#......#",
      "#......#",
      "####D###"
    ],
    warps: {
      "4,5": { toMap: "town3", x: 6, y: 3 }
    }
  },

  gym2: {
    name: "Arène du Feu",
    indoor: true,
    tiles: [
      "########",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "####D###"
    ],
    warps: {
      "4,7": { toMap: "town3", x: 3, y: 3 }
    }
  },

  route4: {
    name: "Route 4",
    tiles: [
      "####################",
      "#..................#",
      '#..""""......""""..#',
      '#..""""......""""..#',
      "<..................>",
      '#..""""......""""..#',
      '#..""""......""""..#',
      "#..................#",
      "#..................#",
      "####################"
    ],
    warps: {
      "0,4": { toMap: "town3", x: 13, y: 4 },
      "19,4": { toMap: "town4", x: 1, y: 4 }
    },
    encounterRate: 0.15,
    encounterTable: [
      { id: 79,  min: 13, max: 16 },  // Ramoloss
      { id: 118, min: 12, max: 15 },  // Poissirene
      { id: 41,  min: 13, max: 16 },  // Nosferapti
      { id: 74,  min: 12, max: 15 },  // Racaillou
      { id: 19,  min: 11, max: 14 }   // Rattata
    ]
  },

  town4: {
    name: "Miréclat",
    tiles: [
      "##############",
      "#............#",
      "#..D..C..M...#",
      "#............#",
      "<............>",
      "#............#",
      "#............#",
      "#............#",
      "#............#",
      "##############"
    ],
    warps: {
      "0,4": { toMap: "route4", x: 18, y: 4 },
      "3,2": { toMap: "gym3", x: 4, y: 6 },
      "6,2": { toMap: "center4", x: 4, y: 4 },
      "13,4": { toMap: "route5", x: 1, y: 4 }
    }
  },

  center4: {
    name: "Centre Pokémon",
    indoor: true,
    tiles: [
      "########",
      "#F..R..#",
      "#..H.P.W",
      "#......#",
      "#......#",
      "####D###"
    ],
    warps: {
      "4,5": { toMap: "town4", x: 6, y: 3 }
    }
  },

  gym3: {
    name: "Arène de l'Eau",
    indoor: true,
    tiles: [
      "########",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "####D###"
    ],
    warps: {
      "4,7": { toMap: "town4", x: 3, y: 3 }
    }
  },

  route5: {
    name: "Route 5",
    tiles: [
      "####################",
      "#..................#",
      '#..""""......""""..#',
      '#..""""......""""..#',
      "<..................>",
      '#..""""......""""..#',
      '#..""""......""""..#',
      "#..................#",
      "#..................#",
      "####################"
    ],
    warps: {
      "0,4": { toMap: "town4", x: 13, y: 4 },
      "19,4": { toMap: "town5", x: 1, y: 4 }
    },
    encounterRate: 0.15,
    encounterTable: [
      { id: 43,  min: 18, max: 21 },  // Mystherbe
      { id: 69,  min: 18, max: 21 },  // Chetiflor
      { id: 102, min: 18, max: 21 },  // Saquedeneu
      { id: 16,  min: 17, max: 20 },  // Roucool
      { id: 27,  min: 17, max: 20 }   // Sabelette
    ]
  },

  town5: {
    name: "Verdeterre",
    tiles: [
      "##############",
      "#............#",
      "#..D..C..M...#",
      "#............#",
      "<............>",
      "#............#",
      "#............#",
      "#............#",
      "#............#",
      "##############"
    ],
    warps: {
      "0,4": { toMap: "route5", x: 18, y: 4 },
      "3,2": { toMap: "gym4", x: 4, y: 6 },
      "6,2": { toMap: "center5", x: 4, y: 4 },
      "13,4": { toMap: "route6", x: 1, y: 4 }
    }
  },

  center5: {
    name: "Centre Pokémon",
    indoor: true,
    tiles: [
      "########",
      "#F..R..#",
      "#..H.P.W",
      "#......#",
      "#......#",
      "####D###"
    ],
    warps: {
      "4,5": { toMap: "town5", x: 6, y: 3 }
    }
  },

  gym4: {
    name: "Arène de la Nature",
    indoor: true,
    tiles: [
      "########",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "####D###"
    ],
    warps: {
      "4,7": { toMap: "town5", x: 3, y: 3 }
    }
  },

  // Route à embranchement: le couloir principal (rangée y=4) mène à la grotte,
  // mais une brèche dans le mur du bas (x=10,y=7) ouvre sur une impasse isolée.
  route6: {
    name: "Route 6",
    tiles: [
      "####################",
      "#..................#",
      '#.""""......""""...#',
      '#.""""......""""...#',
      "<..................D",
      '#.""""......""""...#',
      '#.""""......""""...#',
      "##########.#########",
      "#..................#",
      "####################"
    ],
    warps: {
      "0,4": { toMap: "town5", x: 13, y: 4 },
      "19,4": { toMap: "cave1", x: 1, y: 4 }
    },
    encounterRate: 0.15,
    encounterTable: [
      { id: 95, min: 20, max: 23 },  // Onix
      { id: 66, min: 19, max: 22 },  // Machoc
      { id: 74, min: 19, max: 22 },  // Racaillou
      { id: 19, min: 17, max: 20 },  // Rattata
      { id: 16, min: 17, max: 20 }   // Roucool
    ]
  },

  cave1: {
    name: "Grotte Sombre",
    indoor: true,
    cave: true,
    tiles: [
      "############",
      "#..........#",
      "#..........#",
      "#..........#",
      "D..........D",
      "#..........#",
      "#..........#",
      "############"
    ],
    warps: {
      "0,4": { toMap: "route6", x: 18, y: 4 },
      "11,4": { toMap: "route7", x: 1, y: 4 }
    },
    encounterRate: 0.18,
    encounterTable: [
      { id: 95,  min: 20, max: 24 },  // Onix
      { id: 41,  min: 19, max: 22 },  // Nosferapti
      { id: 66,  min: 19, max: 23 },  // Machoc
      { id: 74,  min: 18, max: 22 }   // Racaillou
    ]
  },

  route7: {
    name: "Route 7",
    tiles: [
      "####################",
      "#..................#",
      '#.""""......""""...#',
      '#.""""......""""...#',
      "<..................>",
      '#.""""......""""...#',
      '#.""""......""""...#',
      "#..................#",
      "#..................#",
      "####################"
    ],
    warps: {
      "0,4": { toMap: "cave1", x: 10, y: 4 },
      "19,4": { toMap: "town6", x: 1, y: 4 }
    },
    encounterRate: 0.15,
    encounterTable: [
      { id: 92,  min: 22, max: 25 },  // Fantominus
      { id: 41,  min: 21, max: 24 },  // Nosferapti
      { id: 19,  min: 19, max: 22 },  // Rattata
      { id: 16,  min: 19, max: 22 },  // Roucool
      { id: 27,  min: 19, max: 22 }   // Sabelette
    ]
  },

  town6: {
    name: "Nocterme",
    tiles: [
      "##################",
      "#................#",
      "#..D..C..M..D....#",
      "#................#",
      "<................>",
      "#................#",
      "#................#",
      "#................#",
      "#................#",
      "##################"
    ],
    warps: {
      "0,4": { toMap: "route7", x: 18, y: 4 },
      "3,2": { toMap: "gym5", x: 4, y: 6 },
      "6,2": { toMap: "center6", x: 4, y: 4 },
      "12,2": { toMap: "maison_inutile1", x: 3, y: 4 },
      "17,4": { toMap: "route8", x: 1, y: 4 }
    }
  },

  center6: {
    name: "Centre Pokémon",
    indoor: true,
    tiles: [
      "########",
      "#F..R..#",
      "#..H.P.W",
      "#......#",
      "#......#",
      "####D###"
    ],
    warps: {
      "4,5": { toMap: "town6", x: 6, y: 3 }
    }
  },

  gym5: {
    name: "Arène de l'Ombre",
    indoor: true,
    tiles: [
      "########",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "####D###"
    ],
    warps: {
      "4,7": { toMap: "town6", x: 3, y: 3 }
    }
  },

  maison_inutile1: {
    name: "Maison",
    indoor: true,
    tiles: [
      "########",
      "#F..R..#",
      "#......W",
      "#......#",
      "#......#",
      "####D###"
    ],
    warps: {
      "4,5": { toMap: "town6", x: 12, y: 3 }
    }
  },

  // Route côtière: bordée d'eau (bloquée, purement décorative) au lieu d'arbres.
  route8: {
    name: "Route Côtière",
    tiles: [
      "~~~~~~~~~~~~~~~~~~~~",
      "~..................~",
      "~..................~",
      "~..................~",
      "<..................>",
      "~..................~",
      "~..................~",
      "~..................~",
      "~..................~",
      "~~~~~~~~~~~~~~~~~~~~"
    ],
    warps: {
      "0,4": { toMap: "town6", x: 17, y: 4 },
      "19,4": { toMap: "town7", x: 1, y: 4 }
    },
    encounterRate: 0.15,
    encounterTable: [
      { id: 118, min: 24, max: 27 },  // Poissirène
      { id: 79,  min: 24, max: 27 },  // Ramoloss
      { id: 60,  min: 23, max: 26 },  // Ptitard
      { id: 41,  min: 23, max: 26 },  // Nosferapti
      { id: 19,  min: 21, max: 24 }   // Rattata
    ]
  },

  town7: {
    name: "Voltis",
    tiles: [
      "##################",
      "#................#",
      "#..D..C..M..D....#",
      "#................#",
      "<................>",
      "#................#",
      "#................#",
      "#................#",
      "#................#",
      "##################"
    ],
    warps: {
      "0,4": { toMap: "route8", x: 18, y: 4 },
      "3,2": { toMap: "gym6", x: 4, y: 6 },
      "6,2": { toMap: "center7", x: 4, y: 4 },
      "12,2": { toMap: "maison_inutile2", x: 3, y: 4 },
      "17,4": { toMap: "route9", x: 1, y: 4 }
    }
  },

  center7: {
    name: "Centre Pokémon",
    indoor: true,
    tiles: [
      "########",
      "#F..R..#",
      "#..H.P.W",
      "#......#",
      "#......#",
      "####D###"
    ],
    warps: {
      "4,5": { toMap: "town7", x: 6, y: 3 }
    }
  },

  gym6: {
    name: "Arène de l'Éclair",
    indoor: true,
    tiles: [
      "########",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "####D###"
    ],
    warps: {
      "4,7": { toMap: "town7", x: 3, y: 3 }
    }
  },

  maison_inutile2: {
    name: "Maison",
    indoor: true,
    tiles: [
      "########",
      "#F..R..#",
      "#......W",
      "#......#",
      "#......#",
      "####D###"
    ],
    warps: {
      "4,5": { toMap: "town7", x: 12, y: 3 }
    }
  },

  route9: {
    name: "Route 9",
    tiles: [
      "####################",
      "#..................#",
      '#.""""......""""...#',
      '#.""""......""""...#',
      "<..................>",
      '#.""""......""""...#',
      '#.""""......""""...#',
      "#..................#",
      "#..................#",
      "####################"
    ],
    warps: {
      "0,4": { toMap: "town7", x: 17, y: 4 },
      "19,4": { toMap: "town8", x: 1, y: 4 }
    },
    encounterRate: 0.15,
    encounterTable: [
      { id: 86,  min: 36, max: 39 },  // Otaria
      { id: 131, min: 36, max: 39 },  // Lokhlass
      { id: 95,  min: 35, max: 38 },  // Onix
      { id: 27,  min: 34, max: 37 },  // Sabelette
      { id: 19,  min: 33, max: 36 }   // Rattata
    ]
  },

  town8: {
    name: "Glacéria",
    tiles: [
      "##################",
      "#................#",
      "#..D..C..M..D....#",
      "#................#",
      "<................>",
      "#................#",
      "#................#",
      "#................#",
      "#................#",
      "##################"
    ],
    warps: {
      "0,4": { toMap: "route9", x: 18, y: 4 },
      "3,2": { toMap: "gym7", x: 4, y: 6 },
      "6,2": { toMap: "center8", x: 4, y: 4 },
      "12,2": { toMap: "maison_inutile3", x: 3, y: 4 },
      "17,4": { toMap: "route10", x: 1, y: 4 }
    }
  },

  center8: {
    name: "Centre Pokémon",
    indoor: true,
    tiles: [
      "########",
      "#F..R..#",
      "#..H.P.W",
      "#......#",
      "#......#",
      "####D###"
    ],
    warps: {
      "4,5": { toMap: "town8", x: 6, y: 3 }
    }
  },

  gym7: {
    name: "Arène du Givre",
    indoor: true,
    tiles: [
      "########",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "####D###"
    ],
    warps: {
      "4,7": { toMap: "town8", x: 3, y: 3 }
    }
  },

  maison_inutile3: {
    name: "Maison",
    indoor: true,
    tiles: [
      "########",
      "#F..R..#",
      "#......W",
      "#......#",
      "#......#",
      "####D###"
    ],
    warps: {
      "4,5": { toMap: "town8", x: 12, y: 3 }
    }
  },

  route10: {
    name: "Route 10",
    tiles: [
      "####################",
      "#..................#",
      '#.""""......""""...#',
      '#.""""......""""...#',
      "<..................>",
      '#.""""......""""...#',
      '#.""""......""""...#',
      "#..................#",
      "#..................#",
      "####################"
    ],
    warps: {
      "0,4": { toMap: "town8", x: 17, y: 4 },
      "19,4": { toMap: "town9", x: 1, y: 4 }
    },
    encounterRate: 0.15,
    encounterTable: [
      { id: 74,  min: 40, max: 43 },  // Racaillou
      { id: 75,  min: 40, max: 43 },  // Gravalanch
      { id: 95,  min: 39, max: 42 },  // Onix
      { id: 66,  min: 39, max: 42 },  // Machoc
      { id: 27,  min: 38, max: 41 }   // Sabelette
    ]
  },

  town9: {
    name: "Solhazar",
    tiles: [
      "##################",
      "#................#",
      "#..D..C..M.......#",
      "#................#",
      "<................>",
      "#................#",
      "#................#",
      "#................#",
      "#................#",
      "##################"
    ],
    warps: {
      "0,4": { toMap: "route10", x: 18, y: 4 },
      "3,2": { toMap: "gym8", x: 4, y: 6 },
      "6,2": { toMap: "center9", x: 4, y: 4 },
      "17,4": { toMap: "route11", x: 1, y: 4 }
    }
  },

  center9: {
    name: "Centre Pokémon",
    indoor: true,
    tiles: [
      "########",
      "#F..R..#",
      "#..H.P.W",
      "#......#",
      "#......#",
      "####D###"
    ],
    warps: {
      "4,5": { toMap: "town9", x: 6, y: 3 }
    }
  },

  gym8: {
    name: "Arène de la Roche",
    indoor: true,
    tiles: [
      "########",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "####D###"
    ],
    warps: {
      "4,7": { toMap: "town9", x: 3, y: 3 }
    }
  },

  route11: {
    name: "Route 11",
    tiles: [
      "####################",
      "#..................#",
      '#.""""......""""...#',
      '#.""""......""""...#',
      "<..................>",
      '#.""""......""""...#',
      '#.""""......""""...#',
      "#..................#",
      "#..................#",
      "####################"
    ],
    warps: {
      "0,4": { toMap: "town9", x: 17, y: 4 },
      "19,4": { toMap: "town10", x: 1, y: 4 }
    },
    encounterRate: 0.15,
    encounterTable: [
      { id: 95,  min: 44, max: 47 },  // Onix
      { id: 76,  min: 44, max: 47 },  // Grolem
      { id: 131, min: 44, max: 47 },  // Lokhlass
      { id: 34,  min: 43, max: 46 },  // Nidoking
      { id: 31,  min: 43, max: 46 }   // Nidoqueen
    ]
  },

  town10: {
    name: "Zénithia",
    tiles: [
      "##################",
      "#................#",
      "#.....C..M...D...#",
      "#................#",
      "<................#",
      "#................#",
      "#................>",
      "#................#",
      "#................#",
      "##################"
    ],
    warps: {
      "0,4": { toMap: "route11", x: 18, y: 4 },
      "6,2": { toMap: "center10", x: 4, y: 4 },
      "13,2": { toMap: "league_hall", x: 4, y: 18 },
      "17,6": { toMap: "route12", x: 0, y: 4 }
    }
  },

  center10: {
    name: "Centre Pokémon",
    indoor: true,
    tiles: [
      "########",
      "#F..R..#",
      "#..H.P.W",
      "#......#",
      "#......#",
      "####D###"
    ],
    warps: {
      "4,5": { toMap: "town10", x: 6, y: 3 }
    }
  },

  // Longue salle de la Ligue: le Conseil des 4 puis le Champion s'enchaînent
  // en remontant le couloir, chacun débloqué une fois le précédent battu.
  league_hall: {
    name: "Salle de la Ligue",
    indoor: true,
    tiles: [
      "########",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "####D###"
    ],
    warps: {
      "4,19": { toMap: "town10", x: 13, y: 3 }
    }
  },

  // --- Seconde campagne ---
  route12: {
    name: "Route 12",
    tiles: [
      "####################",
      "#..................#",
      '#.""""......""""...#',
      '#.""""......""""...#',
      "<..................>",
      '#.""""......""""...#',
      '#.""""......""""...#',
      "#..................#",
      "#..................#",
      "####################"
    ],
    warps: {
      "0,4": { toMap: "town10", x: 17, y: 6 },
      "19,4": { toMap: "town11", x: 1, y: 4 }
    },
    encounterRate: 0.15,
    encounterTable: [
      { id: 27,  min: 47, max: 50 },  // Sabelette
      { id: 111, min: 47, max: 50 },  // Rhinocorne
      { id: 128, min: 47, max: 50 },  // Tauros
      { id: 77,  min: 47, max: 50 },  // Ponyta
      { id: 161, min: 47, max: 50 },  // Fouinette
      { id: 179, min: 47, max: 50 }   // Wattouat
    ]
  },

  town11: {
    name: "Cendrelune",
    tiles: [
      "##################",
      "#................#",
      "#.....C..M...D...#",
      "#................#",
      "<................#",
      "#................#",
      "#................>",
      "#................#",
      "#................#",
      "##################"
    ],
    warps: {
      "0,4": { toMap: "route12", x: 18, y: 4 },
      "6,2": { toMap: "center11", x: 4, y: 4 },
      "13,2": { toMap: "gym9", x: 4, y: 6 },
      "17,6": { toMap: "route13", x: 0, y: 4 }
    }
  },

  center11: {
    name: "Centre Pokémon",
    indoor: true,
    tiles: [
      "########",
      "#F..R..#",
      "#..H.P.W",
      "#......#",
      "#......#",
      "####D###"
    ],
    warps: {
      "4,5": { toMap: "town11", x: 6, y: 3 }
    }
  },

  gym9: {
    name: "Arène des Cendres",
    indoor: true,
    tiles: [
      "########",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "####D###"
    ],
    warps: {
      "4,7": { toMap: "town11", x: 13, y: 3 }
    }
  },

  route13: {
    name: "Route Littorale",
    tiles: [
      "####################",
      "#~~~~..............#",
      '#~~~~..""""........#',
      '#~~~~....""""......#',
      "<..................>",
      '#~~~~....""""......#',
      '#~~~~..""""........#',
      "#~~~~..............#",
      "#..................#",
      "####################"
    ],
    warps: {
      "0,4": { toMap: "town11", x: 17, y: 6 },
      "19,4": { toMap: "town12", x: 1, y: 4 }
    },
    encounterRate: 0.15,
    encounterTable: [
      { id: 98,  min: 49, max: 52 },  // Krabby
      { id: 72,  min: 49, max: 52 },  // Tentacool
      { id: 90,  min: 49, max: 52 },  // Kokiyas
      { id: 120, min: 49, max: 52 },  // Stari
      { id: 170, min: 49, max: 52 },  // Loupio
      { id: 211, min: 49, max: 52 }   // Qwilfish
    ]
  },

  town12: {
    name: "Abyssia",
    tiles: [
      "##################",
      "#................#",
      "#.....C..M...D...#",
      "#................#",
      "<................#",
      "#................#",
      "#................>",
      "#..~~~~~~~~~~~~..#",
      "#..~~~~~~~~~~~~..#",
      "##################"
    ],
    warps: {
      "0,4": { toMap: "route13", x: 18, y: 4 },
      "6,2": { toMap: "center12", x: 4, y: 4 },
      "13,2": { toMap: "gym10", x: 4, y: 6 },
      "17,6": { toMap: "route14", x: 0, y: 4 }
    }
  },

  center12: {
    name: "Centre Pokémon",
    indoor: true,
    tiles: [
      "########",
      "#F..R..#",
      "#..H.P.W",
      "#......#",
      "#......#",
      "####D###"
    ],
    warps: {
      "4,5": { toMap: "town12", x: 6, y: 3 }
    }
  },

  gym10: {
    name: "Arène des Marées Noires",
    indoor: true,
    tiles: [
      "########",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "####D###"
    ],
    warps: {
      "4,7": { toMap: "town12", x: 13, y: 3 }
    }
  },

  route14: {
    name: "Route 14",
    tiles: [
      "####################",
      "#..................#",
      '#.""""......""""...#',
      '#.""""......""""...#',
      "<..................>",
      '#.""""......""""...#',
      '#.""""......""""...#',
      "#..................#",
      "#..................#",
      "####################"
    ],
    warps: {
      "0,4": { toMap: "town12", x: 17, y: 6 },
      "19,4": { toMap: "town13", x: 1, y: 4 }
    },
    encounterRate: 0.15,
    encounterTable: [
      { id: 84,  min: 53, max: 56 },  // Doduo
      { id: 41,  min: 53, max: 56 },  // Nosferapti
      { id: 111, min: 53, max: 56 },  // Rhinocorne
      { id: 128, min: 53, max: 56 },  // Tauros
      { id: 163, min: 53, max: 56 },  // Hoothoot
      { id: 165, min: 53, max: 56 }   // Coxy
    ]
  },

  town13: {
    name: "Auroria",
    tiles: [
      "##################",
      "#................#",
      "#.....C..M...D...#",
      "#................#",
      "<................#",
      "#................#",
      "#................>",
      "#................#",
      "#................#",
      "##################"
    ],
    warps: {
      "0,4": { toMap: "route14", x: 18, y: 4 },
      "6,2": { toMap: "center13", x: 4, y: 4 },
      "13,2": { toMap: "gym11", x: 4, y: 6 },
      "17,6": { toMap: "route15", x: 0, y: 4 }
    }
  },

  center13: {
    name: "Centre Pokémon",
    indoor: true,
    tiles: [
      "########",
      "#F..R..#",
      "#..H.P.W",
      "#......#",
      "#......#",
      "####D###"
    ],
    warps: {
      "4,5": { toMap: "town13", x: 6, y: 3 }
    }
  },

  gym11: {
    name: "Arène du Ciel",
    indoor: true,
    tiles: [
      "########",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "####D###"
    ],
    warps: {
      "4,7": { toMap: "town13", x: 13, y: 3 }
    }
  },

  route15: {
    name: "Route 15",
    tiles: [
      "####################",
      "#..................#",
      '#.""""......""""...#',
      '#.""""......""""...#',
      "<..................>",
      '#.""""......""""...#',
      '#.""""......""""...#',
      "#..................#",
      "#..................#",
      "####################"
    ],
    warps: {
      "0,4": { toMap: "town13", x: 17, y: 6 },
      "19,4": { toMap: "town14", x: 1, y: 4 }
    },
    encounterRate: 0.15,
    encounterTable: [
      { id: 66,  min: 55, max: 58 },  // Machoc
      { id: 56,  min: 55, max: 58 },  // Férosinge
      { id: 111, min: 55, max: 58 },  // Rhinocorne
      { id: 74,  min: 55, max: 58 },  // Racaillou
      { id: 236, min: 55, max: 58 },  // Debugant
      { id: 246, min: 55, max: 58 }   // Embrylex
    ]
  },

  town14: {
    name: "Obsidor",
    tiles: [
      "##################",
      "#................#",
      "#.....C..M...D...#",
      "#................#",
      "<................#",
      "#................#",
      "#................>",
      "#................#",
      "#................#",
      "##################"
    ],
    warps: {
      "0,4": { toMap: "route15", x: 18, y: 4 },
      "6,2": { toMap: "center14", x: 4, y: 4 },
      "13,2": { toMap: "gym12", x: 4, y: 6 },
      "17,6": { toMap: "route16", x: 0, y: 4 }
    }
  },

  center14: {
    name: "Centre Pokémon",
    indoor: true,
    tiles: [
      "########",
      "#F..R..#",
      "#..H.P.W",
      "#......#",
      "#......#",
      "####D###"
    ],
    warps: {
      "4,5": { toMap: "town14", x: 6, y: 3 }
    }
  },

  gym12: {
    name: "Arène des Failles",
    indoor: true,
    tiles: [
      "########",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "####D###"
    ],
    warps: {
      "4,7": { toMap: "town14", x: 13, y: 3 }
    }
  },

  route16: {
    name: "Route 16",
    tiles: [
      "####################",
      "#..................#",
      '#.""""......""""...#',
      '#.""""......""""...#',
      "<..................>",
      '#.""""......""""...#',
      '#.""""......""""...#',
      "#..................#",
      "#..................#",
      "####################"
    ],
    warps: {
      "0,4": { toMap: "town14", x: 17, y: 6 },
      "19,4": { toMap: "town15", x: 1, y: 4 }
    },
    encounterRate: 0.15,
    encounterTable: [
      { id: 96,  min: 57, max: 60 },  // Soporifik
      { id: 102, min: 57, max: 60 },  // Noeunoeuf
      { id: 56,  min: 57, max: 60 },  // Férosinge
      { id: 128, min: 57, max: 60 },  // Tauros
      { id: 177, min: 57, max: 60 },  // Natu
      { id: 203, min: 57, max: 60 }   // Girafarig
    ]
  },

  town15: {
    name: "Floramne",
    tiles: [
      "##################",
      "#................#",
      "#.....C..M...D...#",
      "#................#",
      "<................#",
      "#................#",
      "#................>",
      "#................#",
      "#................#",
      "##################"
    ],
    warps: {
      "0,4": { toMap: "route16", x: 18, y: 4 },
      "6,2": { toMap: "center15", x: 4, y: 4 },
      "13,2": { toMap: "gym13", x: 4, y: 6 },
      "17,6": { toMap: "route17", x: 0, y: 4 }
    }
  },

  center15: {
    name: "Centre Pokémon",
    indoor: true,
    tiles: [
      "########",
      "#F..R..#",
      "#..H.P.W",
      "#......#",
      "#......#",
      "####D###"
    ],
    warps: {
      "4,5": { toMap: "town15", x: 6, y: 3 }
    }
  },

  gym13: {
    name: "Arène des Songes",
    indoor: true,
    tiles: [
      "########",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "####D###"
    ],
    warps: {
      "4,7": { toMap: "town15", x: 13, y: 3 }
    }
  },

  route17: {
    name: "Route 17",
    tiles: [
      "####################",
      "#..................#",
      '#.""""......""""...#',
      '#.""""......""""...#',
      "<..................>",
      '#.""""......""""...#',
      '#.""""......""""...#',
      "#..................#",
      "#..................#",
      "####################"
    ],
    warps: {
      "0,4": { toMap: "town15", x: 17, y: 6 },
      "19,4": { toMap: "town16", x: 1, y: 4 }
    },
    encounterRate: 0.15,
    encounterTable: [
      { id: 81,  min: 59, max: 62 },  // Magnéti
      { id: 74,  min: 59, max: 62 },  // Racaillou
      { id: 111, min: 59, max: 62 },  // Rhinocorne
      { id: 56,  min: 59, max: 62 },  // Férosinge
      { id: 204, min: 59, max: 62 },  // Pomdepik
      { id: 213, min: 59, max: 62 }   // Caratroc
    ]
  },

  town16: {
    name: "Ferrance",
    tiles: [
      "##################",
      "#................#",
      "#.....C..M...D...#",
      "#................#",
      "<................#",
      "#................#",
      "#................>",
      "#................#",
      "#................#",
      "##################"
    ],
    warps: {
      "0,4": { toMap: "route17", x: 18, y: 4 },
      "6,2": { toMap: "center16", x: 4, y: 4 },
      "13,2": { toMap: "gym14", x: 4, y: 6 },
      "17,6": { toMap: "route18", x: 0, y: 4 }
    }
  },

  center16: {
    name: "Centre Pokémon",
    indoor: true,
    tiles: [
      "########",
      "#F..R..#",
      "#..H.P.W",
      "#......#",
      "#......#",
      "####D###"
    ],
    warps: {
      "4,5": { toMap: "town16", x: 6, y: 3 }
    }
  },

  gym14: {
    name: "Arène de l'Acier",
    indoor: true,
    tiles: [
      "########",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "####D###"
    ],
    warps: {
      "4,7": { toMap: "town16", x: 13, y: 3 }
    }
  },

  route18: {
    name: "Route Nocturne",
    tiles: [
      "####################",
      "#~~~~..............#",
      "#~~~~..............#",
      "#~~~~...........~~~#",
      "<..................>",
      "#~~~~...........~~~#",
      "#~~~~..............#",
      "#~~~~..............#",
      "#..................#",
      "####################"
    ],
    warps: {
      "0,4": { toMap: "town16", x: 17, y: 6 },
      "19,4": { toMap: "town17", x: 1, y: 4 }
    },
    encounterRate: 0.15,
    encounterTable: [
      { id: 91,  min: 61, max: 64 },  // Crustabri
      { id: 90,  min: 61, max: 64 },  // Kokiyas
      { id: 120, min: 61, max: 64 },  // Stari
      { id: 41,  min: 61, max: 64 },  // Nosferapti
      { id: 222, min: 61, max: 64 },  // Corayon
      { id: 223, min: 61, max: 64 }   // Rémoraid
    ]
  },

  town17: {
    name: "Lunécume",
    tiles: [
      "##################",
      "#................#",
      "#.....C..M...D...#",
      "#................#",
      "<................#",
      "#................#",
      "#................>",
      "#..~~~~~~~~~~~~..#",
      "#..~~~~~~~~~~~~..#",
      "##################"
    ],
    warps: {
      "0,4": { toMap: "route18", x: 18, y: 4 },
      "6,2": { toMap: "center17", x: 4, y: 4 },
      "13,2": { toMap: "gym15", x: 4, y: 6 },
      "17,6": { toMap: "route19", x: 0, y: 4 }
    }
  },

  center17: {
    name: "Centre Pokémon",
    indoor: true,
    tiles: [
      "########",
      "#F..R..#",
      "#..H.P.W",
      "#......#",
      "#......#",
      "####D###"
    ],
    warps: {
      "4,5": { toMap: "town17", x: 6, y: 3 }
    }
  },

  gym15: {
    name: "Arène des Marées Lunaires",
    indoor: true,
    tiles: [
      "########",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "####D###"
    ],
    warps: {
      "4,7": { toMap: "town17", x: 13, y: 3 }
    }
  },

  // --- Astrae : dernière étape de la seconde campagne ---
  route19: {
    name: "Route d'Astrae",
    tiles: [
      "####################",
      "#..................#",
      "#..................#",
      "#..................#",
      "<..................>",
      "#..................#",
      "#..................#",
      "#..................#",
      "#..................#",
      "####################"
    ],
    warps: {
      "0,4": { toMap: "town17", x: 17, y: 6 },
      "19,4": { toMap: "town18", x: 1, y: 4 }
    },
    encounterRate: 0.12,
    encounterTable: [
      { id: 24,  min: 63, max: 66 },  // Arbok
      { id: 53,  min: 63, max: 66 },  // Persian
      { id: 91,  min: 63, max: 66 },  // Crustabri
      { id: 82,  min: 63, max: 66 },  // Magnéton
      { id: 198, min: 63, max: 66 },  // Cornèbre
      { id: 214, min: 63, max: 66 }   // Scarhino
    ]
  },

  town18: {
    name: "Astrae",
    tiles: [
      "##################",
      "#................#",
      "#........C...M...#",
      "#................#",
      "<................#",
      "#................#",
      "#........D.......#",
      "#................#",
      "#........D.......#",
      "##################"
    ],
    warps: {
      "0,4": { toMap: "route19", x: 18, y: 4 },
      "9,2": { toMap: "center18", x: 4, y: 4 },
      "9,6": { toMap: "gym16", x: 4, y: 15 },
      "9,8": { toMap: "super_league_hall", x: 4, y: 23 }
    }
  },

  center18: {
    name: "Centre Pokémon",
    indoor: true,
    tiles: [
      "########",
      "#F..R..#",
      "#..H.P.W",
      "#......#",
      "#......#",
      "####D###"
    ],
    warps: {
      "4,5": { toMap: "town18", x: 9, y: 5 }
    }
  },

  // Long sanctuaire: l'opérative de la Main Noire, puis leur cheffe tout au fond.
  gym16: {
    name: "Sanctuaire d'Astrae",
    indoor: true,
    tiles: [
      "########",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "####D###"
    ],
    warps: {
      "4,15": { toMap: "town18", x: 9, y: 7 }
    }
  },

  // Super Ligue: un long sanctuaire souterrain, quatre Super-Membres du
  // Conseil puis la cheffe de la Main Noire tout au sommet, pour la vraie
  // conclusion de cette seconde campagne.
  super_league_hall: {
    name: "Sanctuaire Profond",
    indoor: true,
    cave: true,
    tiles: [
      "########",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "#......#",
      "####D###"
    ],
    warps: {
      "4,23": { toMap: "town18", x: 9, y: 7 }
    }
  }
};

PKMN.START_MAP = "town";
